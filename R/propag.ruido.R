...
#' @param Alfa Matrix with values of  `alfa` constant of the decay equation (see details for more information). Need to be one row for each point, even if they are the same.
#' @param Beta Same as the alfa parameter, but representing values of `beta` constant.
#' @param elev.r Raster with elevation values. OBS.: This raster will be used to extract all relevant GIS information (ex.: reslution, size, crs).
...
#' @param save.meta Logical. If `TRUE` all steps of the process of modeling will be saved in your work directory. Objects will be saved with 'Results.propag' at the beggining of the file name. (By default = `TRUE`)
#' @param name.meta Optional. Define a custom name to be added after de default name if `save.meta` is `TRUE`.
#' @param multicore Logical. If `TRUE` multicore processing will be active. This parameter improve the processing time. (by deafult = `TRUE`)
#' @param cl Numerical. Cluster object. Number of processes to be used in the `propag.ruido` computation. Only considered if `multicore` is `TRUE`. See documentation of package `parallel` and `snow` for details. (By default = number of CPU cores of the current host less one)
#'
#'
#'
