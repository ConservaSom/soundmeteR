...
#Modificado a partir da propag.ruido para poder ser implementada em fondes de ruído em forma de linha (ex. estradas e esteiras)
#2018.05.31
#versao 0.1
###################################################################################################################

#OBSERVAÇÕES ####
#implementado apenas para linhas retas até o momento. Com apenas duas coordenadas: Inicio e fim da linha.
#No futuro, implementar para linhas a partir de um shape


####Argumentos ####
#line.origin: Coordenadas da origem da linha (obrigatóriamente uma reta).
#
##line.end: Coordenadas do final da linha (obrigatóriamente uma reta).
#
...
propag.ruido.linha <-function(line.origin=NA, line.end=NA, ID=NULL, NIS0=NA, Alfa=NA, Beta=NA, elev.r=NA, contour.veg=NA, save.meta=T, name.meta=NULL, multicore=F, cl=detectCores()-1){
  ...
  for (i in 1:nrow(line.origin)) {
    ...
    elev.rel<-elev.r #Resolvido por hora, pensar numa mudança junto com o apply depois ####
    ...
    if(multicore){ #implementando multicore ####
      ...
      clusterExport(cl.int, list("SpatialLines", "Lines", "Line", "crs","crs<-","SpatialLinesLengths", "gIntersection", "gDistance", "SpatialPoints")) #enviando as funções necessárias para os processos
